...
# Volume
volume_and_muted=$(amixer -D pulse get Master | awk -F '[\[|\]|%]' 'NR==6 { print $2, $5 }')
volume=$(echo $volume_and_muted | cut -d' ' -f1)
sound=$(echo $volume_and_muted | cut -d' ' -f2)
